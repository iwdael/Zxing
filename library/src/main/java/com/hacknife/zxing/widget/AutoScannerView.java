...
public class AutoScannerView extends View {
    ...
    protected Paint traAnglePaint;
    protected Paint textPaint;
    protected CameraManager cameraManager;

    protected int maskColor = Color.parseColor("#60000000");//蒙在摄像头上面区域的半透明颜色
    protected int triAngleColor = Color.parseColor("#76EE00");    //边角的颜色
    protected int lineColor = Color.parseColor("#FF0000");   //中间线的颜色
    protected int textColor = Color.parseColor("#CCCCCC"); //文字的颜色
    protected Drawable scannerDrawable;
    protected int triAngleLength = dp2px(20);                                         //每个角的点距离
    protected int triAngleWidth = dp2px(4);                                           //每个角的点宽度
    protected int textMarinTop = dp2px(30);                                           //文字距离识别框的距离
    protected int lineOffsetCount = 0;

    public AutoScannerView(Context context) {
        this(context, null);
    }

    public AutoScannerView(Context context, AttributeSet attrs) {
        super(context, attrs);
        TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.AutoScannerView);
        maskColor = ta.getColor(R.styleable.AutoScannerView_maskColor, maskColor);
        triAngleColor = ta.getColor(R.styleable.AutoScannerView_triAngleColor, triAngleColor);
        lineColor = ta.getColor(R.styleable.AutoScannerView_lineColor, lineColor);
        textColor = ta.getColor(R.styleable.AutoScannerView_textColor, textColor);
        scannerDrawable = ta.getDrawable(R.styleable.AutoScannerView_scannerDrawable);
...
//            canvas.drawLine(frame.left, frame.top + lineOffsetCount, frame.right, frame.top + lineOffsetCount, linePaint);    //画一条红色的线
            ...
            canvas.drawBitmap(((BitmapDrawable) scannerDrawable).getBitmap(), null, lineRect, linePaint);
        }
